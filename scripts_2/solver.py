...
data_filename = os.path.join(os.path.dirname(__file__), 'HConVRPDatasets_YML', 'Medium', '15%', 'b10.yml') # '15%', 
...
class VRPSolver:
    def __init__(self, vehicles, customers, depot, planning_horizon, distance_matrix, max_route_duration, solution):
        ...
        for period in range(self.planning_horizon):
            ...
            for vehicle in self.vehicles:
                ...
                for i in range(len(route) - 1):
                    start_node = route[i]
                    end_node = route[i+1]
                    period_cost += self.distance_matrix[start_node.id][end_node.id] * vehicle.variable_cost
    ...
    def relocation_optimization(self, max_iterations=10):
        """
...
for i in range(10):
    condition = solver.relocation_optimization()
    if not condition:
        break
solver.solution.write_solution("solutions/solution.yml")  
print(solver.solution_df)
# show only the row with the lowest total cost
print(solver.solution_df[solver.solution_df["TotalCost"] == solver.solution_df["TotalCost"].min()])
