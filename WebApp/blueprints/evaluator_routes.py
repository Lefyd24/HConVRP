...
def evaluate():
    solution_files = []
    for file in os.listdir(current_app.config['SOLUTION_PATH']):
        if file.endswith('.yml'):
            solution_files.append(file)
    return render_template('evaluate.html', solution_files=solution_files)

@evaluator_bp.route('/evaluate/<solution_file>', methods=['GET'])
def evaluate_solution(solution_file):
    ...
    if view_mode == 'true':
        with open(os.path.join(current_app.config['SOLUTION_PATH'], solution_file), 'r') as file:
        ...
        start = time.time()
        solution = loader.load_solution(solution_file)
        ...
        evaluation_results = {
            ...
            'execution_time': f"<span class='text-success'>{round(end-start, 3)}''</span>"
        }
