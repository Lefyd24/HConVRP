...
import random
from models import *
from neighborhoods import RelocationNeighborhood, SwapNeighborhood
import yaml
import os
import numpy as np
...
class HConVRP:
    ...
    def _update_solution_routes(self, step:str):
        """
        Helper function to update the solution object with the current routes.
        """
        self.solution.routes = {}
        for period in range(self.planning_horizon):
            for vehicle in self.vehicles:
                self.solution.add_route(period, vehicle, vehicle.routes[period])
        # Recalculate the total cost after the relocations
        for period in range(self.planning_horizon):
            total_cost = sum(vehicle.cost[period] for vehicle in self.vehicles)
            self.solution.total_cost[period] = float(total_cost)
        self.solution_df = pd.concat([self.solution_df, pd.DataFrame([[step] + list(self.solution.total_cost.values()) + [sum(self.solution.total_cost.values())]], columns=self.solution_df.columns)], ignore_index=True)
    ...
    def initial_assignment(self):
        ...
        self._update_solution_routes("Initial")
        
        # Calculate the total cost of the initial solution
        final_cost = self.objective_function()

  
    def relocation_optimization(self, start_time, max_iterations=50):
        """
        ...
        print(f"Total intra relocations: {total_intra_relocations} | Total inter relocations: {total_inter_relocations} | Total inter frequent relocations: {total_inter_frequent_relocations}")
        
        # Update the solution object with the new routes
        self._update_solution_routes("Relocation")
        

    def swap_optimization(self, start_time, max_iterations=40):
        """
            ...
            for period in range(self.planning_horizon):
                for vehicle in self.vehicles:
                    for other_vehicle in self.vehicles:
                        ...
                        # Find the best swap
                        best_swap, best_swap_is_frequent = self.swap_neighborhood.find_best_swap(vehicle, other_vehicle, period, self.tabu_search, current_cost)
        ...
        print(f"Total intra-swaps: {total_intra_swaps} | Total inter-swaps: {total_inter_swaps} | Total frequent swaps: {total_frequent_swaps}")
        # Update the solution object with the new routes
        self._update_solution_routes("Swap")
        
        return total_intra_swaps + total_frequent_swaps + total_inter_swaps

    
    def change_vehicle_chain_optimization(self, start_time):
        """
        ...
        # print(f"Total chain relocations: {total_chain_relocations}")
        
        # Update the solution object with the new routes
        self._update_solution_routes("ChangeVehicleChain")
        ...
        for first_route_node_index in range(1, len(vehicle_from.routes[period]) - 1):
            ...
            for second_route_node_index in range(1, len(vehicle_middle.routes[period]) - 1):
                ...
                for third_route_node_index in range(1, len(vehicle_to.routes[period]) - 1):
                    ...
                    vehicle_middle_copy = vehicle_middle.shallow_copy() #deepcopy(vehicle_middle)
    ...
    def two_opt_optimization(self, start_time, max_iterations=50):
        """
        ...
        print(f"2-opt Iteration concluded within {iteration} iterations - Improvements: {total_2_opt_improvements}")
    ...
    def or_opt_optimization(self, start_time, max_iterations=50):
        """
        ...
        # Emit progress update
        print(f"Or-opt concluded within {iteration} iterations - Improvements: {total_or_opt_improvements}")
